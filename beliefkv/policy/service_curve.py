...
class TransferServiceCurve:
    ...
    def observe(self, telemetry: TransferTelemetry) -> None:
        ...
        key = ServiceCurveKey(
            telemetry.direction,
            self.size_bucket(max(telemetry.actual_bytes, telemetry.closure_bytes)),
            telemetry.compute_phase or "unknown",
        )
        ...
        effective_rate = None
        if callback_ms is not None and callback_ms > 0 and telemetry.actual_bytes > 0:
            effective_rate = telemetry.actual_bytes / callback_ms
    ...
    def estimate(
        ...
        *,
        ...
        key = ServiceCurveKey(
            direction, self.size_bucket(size_bytes), compute_phase or "unknown"
        )
        ...
        callback_ms = max(
            ...
            setup_p90_ms + (size_bytes / rate_p10 if size_bytes else 0.0),
        )
        compute_wait = [
